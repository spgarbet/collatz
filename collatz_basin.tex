...
\]
equivalently, the set of all positive integers of the form $m - 1$ or $m$
where $m$ is a \emph{3-smooth number} (a number whose only prime factors are
$2$ and~$3$).
...
\begin{definition}
  ...
  \[
    \basin
    \;=\;
    \bigl\{\, m - 1 \;:\; m \in \smooth,\; m \geq 2 \,\bigr\}
    \;\cup\;
    \bigl\{\, 2^a \;:\; a \geq 1 \,\bigr\}.
  \]
  Equivalently,
  \[
    \basin
    \;=\;
    \bigl\{\, 2^a 3^b - 1 \;:\; a \geq 1,\; b \geq 0 \,\bigr\}
    \;\cup\;
    \bigl\{\, 2^a \;:\; a \geq 1 \,\bigr\}.
  \]
\end{definition}
...
\[
  1,\;2,\;3,\;4,\;5,\;7,\;8,\;11,\;15,\;16,\;17,\;23,\;31,\;32,
\]
\[
  35,\;47,\;53,\;63,\;64,\;71,\;\ldots
\]
...
Sections~\ref{sec:powers2}--\ref{sec:induction}, and the closed form is
established in Section~\ref{sec:closedform}.
...
\begin{remark}
  The basin $\basin$ is equivalent to the set
  \[
    \basin = \bigl\{\, m - \epsilon \;:\;
               m = 2^a 3^b \in \smooth,\;
               \epsilon \in \{0, 1\},\;
               m - \epsilon \geq 1 \,\bigr\}
  \]
  after observing that $\epsilon = 0$ recovers the powers-of-two component
  (since $b = 0$ must hold for a power of $2$ to itself be in $\smooth$),
  and $\epsilon = 1$ recovers the odd component.
  Informally: $\basin$ consists of every 3-smooth number and every
  positive integer that is one less than a 3-smooth number.
\end{remark}
